...
const init = function () {
    const messages = [
        ...
        {prop: "secretKey", question:"secreate key: ", answer:""}
    ];
        ...
        r.question(messages[index].question, function (answer) {
            ...
            if(index == messages.length-1){
                ...
                fs.writeFile(jsonPath+'blog.json', JSON.stringify(blogJson), 'utf8', function(err){
                    if(err){
                        console.log('An error occurred while issuing blog.json ');
                        throw err;
                    }
                    console.log('Completed blog information\nProceeding with accessToken');
                    getToken();
                });
                return r.close();
            }
...
const downloadAccessToken = function(blogJson) {
    ...
    const getAccessToken = function(code, blogJson) {
        ...
        request(options, function(error, response, body) {
            ...
            if(error){
                console.log('There was a problem in requesting access token from the Tistory API');
                throw error;
            }
            ...
            if(accessToken){
                fs.writeFile(jsonPath+'token.json', JSON.stringify({"accessToken":accessToken}), 'utf8', function(err){
                    if(err) {
                        console.log('There was a problem creating the access token file.\n');
                        throw err;
                    }
...
const findMarkdownFile = function (callback, blogJson, accessToken) {
    ...
    if(markdownFileName){
        ...
        console.log('There are no Markdown files in the current folder.');
    }
};
        ...
        imageConverter.sendImage(data, tokenForSendImage, function (markdown) {
            ...
            request(options, function(error, response, body){
                ...
                if(response.statusCode == 200){
                    console.log('Congratulations!! Posted.');
                }else{
